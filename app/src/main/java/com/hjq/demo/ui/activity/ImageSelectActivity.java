...
public final class ImageSelectActivity extends AppActivity
    ...
    public static final String INTENT_KEY_OUT_IMAGE_LIST = "imageList";
    public static final String IS_ORIGINAL = "IS_ORIGINAL";
    public static final String IS_ORIGINAL_DATA = "IS_ORIGINAL_DATA";
    ...
    public static void start(BaseActivity activity, int maxSelect, boolean canEdit, OnPhotoSelectListener listener) {
        if (maxSelect < 1) {
            // 最少要选择一个图片
        ...
        intent.putExtra(Constant.EDIT_PHOTO, canEdit);
        activity.startActivityForResult(intent, (resultCode, data) -> {
            ...
            boolean isOriginalData = !TextUtils.isEmpty(data.getStringExtra(IS_ORIGINAL_DATA));
            ...
            if (resultCode == RESULT_OK && !list.isEmpty()) {
                if (isOriginalData) {
                    boolean isOriginal = data.getBooleanExtra(IS_ORIGINAL, false);
    ...
    public static void start(BaseActivity activity, int maxSelect, OnPhotoSelectListener listener) {
        start(activity, maxSelect, false, listener);
    }
    ...
    private AppCompatCheckBox rbOriginalButton;
    ...
    private boolean isEditPhotoMode;
    ...
    protected void initView() {
        ...
        mOriginalLayout = findViewById(R.id.ll_original);
        rbOriginalButton = findViewById(R.id.rb_original_check);
        setOnClickListener(mFloatingView);
        mOriginalLayout.setOnClickListener(v -> {
            rbOriginalButton.setChecked(!rbOriginalButton.isChecked());
        });
    ...
    protected void initData() {
        ...
        isEditPhotoMode = getBoolean(Constant.EDIT_PHOTO);
        if (mMaxSelect > 1) {
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.fab_image_select_floating) {
            ...
            Intent data = new Intent()
                    .putStringArrayListExtra(INTENT_KEY_OUT_IMAGE_LIST, mSelectImage);
            if (mOriginalLayout.getVisibility() == View.VISIBLE) {
                data.putExtra(IS_ORIGINAL, rbOriginalButton.isChecked())
                        .putExtra(IS_ORIGINAL_DATA, IS_ORIGINAL_DATA);
            }
    ...
    public void onItemClick(RecyclerView recyclerView, View itemView, int position) {
        if (isEditPhotoMode) {
            ...
            bundle.putBoolean(Constant.EDIT_PHOTO, true);
            ImagePreviewActivity.start(getActivity(), mAdapter.getData(), position, bundle, new OnPhotoSelectListener() {
            ...
            ImagePreviewActivity.start(getActivity(), mAdapter.getData(), position, new Bundle());
        }
    }
    ...
    public interface OnPhotoSelectListener {
        ...
        default void onSelected(List<String> data, boolean isOriginal) {
        }
