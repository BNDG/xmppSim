        ...
        if (pathTime != time) {
            ...
            for (int a = 0, N = endKeyFrame.commands.size(); a < N; a++) {
                ...
                if (endCommand instanceof MoveTo) {
                    ...
                    if (start != null) {
                        path.moveTo(dpf2(start.x + (end.x - start.x) * progress), dpf2(start.y + (end.y - start.y) * progress));
                    } else {
                        path.moveTo(dpf2(end.x), dpf2(end.y));
                    }
                } else if (endCommand instanceof LineTo) {
                    LineTo end = (LineTo) endCommand;
                    LineTo start = (LineTo) startCommand;
                    if (start != null) {
                        path.lineTo(dpf2(start.x + (end.x - start.x) * progress), dpf2(start.y + (end.y - start.y) * progress));
                    } else {
                        path.lineTo(dpf2(end.x), dpf2(end.y));
                    }
                } else if (endCommand instanceof CurveTo) {
                    CurveTo end = (CurveTo) endCommand;
                    CurveTo start = (CurveTo) startCommand;
                    if (start != null) {
                        path.cubicTo(dpf2(start.x1 + (end.x1 - start.x1) * progress), dpf2(start.y1 + (end.y1 - start.y1) * progress),
                                dpf2(start.x2 + (end.x2 - start.x2) * progress), dpf2(start.y2 + (end.y2 - start.y2) * progress),
                                dpf2(start.x + (end.x - start.x) * progress), dpf2(start.y + (end.y - start.y) * progress));
                    } else {
                        path.cubicTo(dpf2(end.x1), dpf2(end.y1), dpf2(end.x2), dpf2(end.y2), dpf2(end.x), dpf2(end.y));
                    }
                }
            }
            path.close();
