...
public class BitmapLoaderUtil {
    ...
    public static String saveBitmapToLocal(Bitmap bitmap, String fileName) {
        ...
        try {
            ...
            imgFile = new File(Constant.PICTURE_DIR, fileName + ".png");
     ...
     * @return 显示在UI上的高宽
     */
    public static int[] getImageWidthAndHeightToTalk(int pic_width, int pic_height) {
        int w;
        int h;
        int[] w_h = new int[2];
        int screeWidth = ScreenUtils.getScreenWidth() / 2;
        int screeHeight = ScreenUtils.getScreenHeight() / 4;
        if (pic_width == pic_height) {
            ...
            if (pic_width > pic_height && pic_width >= screeWidth) {
                ...
                if (h < ScreenUtils.getScreenWidth() / 6) {
                    h = ScreenUtils.getScreenHeight() / 6;
                }
            } else if (pic_height > pic_width && pic_height > screeHeight) {
                h = screeHeight;
                rs = (float) pic_height / (float) screeHeight;
                w = (int) ((float) pic_width / rs);
                if (w < ScreenUtils.getScreenWidth() / 6) {
                    w = ScreenUtils.getScreenHeight() / 6;
                }
            } else {
                w = screeWidth;
                h = screeHeight;
        ...
        if (w < SizeUtils.dp2px(120)) {
            float dis = (float) w / SizeUtils.dp2px(120);
            h = (int) ((1 + (1 - dis)) * h);
            w = SizeUtils.dp2px(120);
        }
        w_h[0] = w;
        w_h[1] = h;
        if (w_h[0] < SizeUtils.dp2px(130f)) {
            w_h[0] = SizeUtils.dp2px(130f);
        }
        return w_h;
    }
}
