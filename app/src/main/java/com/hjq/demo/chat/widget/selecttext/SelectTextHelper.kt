...
import com.hjq.demo.chat.widget.quote.DividerSpan
import com.hjq.demo.chat.widget.quote.QuoteHelper
import com.hjq.demo.chat.widget.quote.QuoteSpan
import com.hjq.demo.utils.MyLinkify
import java.util.*
...
class SelectTextHelper(builder: Builder) {
    ...
    private fun init() {
        ...
        val startsWithQuote: Boolean = handleTextQuotes(mTextView, spanStr)
        if (startsWithQuote) {
            spanStr.setSpan(
                DividerSpan(false),
                0,
...
//        SelectUtils.replaceContent(spanStr, mOriginalContent, "\u0020\u0020", "\u3000\u3000")
     ...
     */
    private fun handleTextQuotes(
        textView: TextView,
        body: SpannableStringBuilder
    ): Boolean {
        var startsWithQuote = false
        var quoteDepth = 1
        while (QuoteHelper.bodyContainsQuoteStart(body) && quoteDepth <= QuoteHelper.QUOTE_MAX_DEPTH) {
            var previous = '\n'
            var lineStart = -1
            var lineTextStart = -1
            var quoteStart = -1
                ...
                if (lineStart == -1) {
                    if (previous == '\n') {
                        if (i < body.length && QuoteHelper.isPositionQuoteStart(body, i)) {
                            ...
                            applyQuoteSpan(textView, body, quoteStart, i - 1)
                            quoteStart = -1
                        }
                    }
                } else {
            ...
            if (quoteStart >= 0) {
                ...
                applyQuoteSpan(textView, body, quoteStart, body.length)
            }
            quoteDepth++
        }
    ...
    private fun applyQuoteSpan(
        ...
        if (start > 1 && "\n\n" != body.subSequence(start - 2, start).toString()) {
            body.insert(start++, "\n")
            body.setSpan(
                DividerSpan(false), start - 2, start, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
            )
            end++
        }
        ...
        body.setSpan(
            QuoteSpan(Color.parseColor("#343434"), metrics),
            start,
            end,
            Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
        )
