...
public class Builder {
    ...
    public void build() {
        ...
        if (bitmap != null) {
            ...
            Log.w(">>>>", "build: 缓存中存在，直接显示，不再生成bitmap了");
        ...
        if (bitmapDisk != null) {
            ...
            // 缓存中存在，直接显示，不再生成bitmap了
            Log.w(">>>>", "磁盘缓存中存在，直接显示，不再生成bitmap了");
        ...
        for (int i = 0; i < count; i++) {
            ...
            } else if (item.contains(context.get().getFilesDir().toString())) {
                loadFileGlide(isDing, i, new File(item));
            } else {
                Message message = mHandler.obtainMessage(LOAD_END, i, 0, getShortNickName(item));
    ...
    private void loadBitmapGlide(boolean isDing, final int position) {
        Log.w(">>>>", "loadBitmapGlide: bug here? ");
        // 同时读取file存在冲突问题
                ...
                Glide.with(context.get())
                        ...
                        .listener(new RequestListener<Bitmap>() {
                            ...
                            public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Bitmap> target,
                                ...
                                Log.w(">>>>", "loadBitmapGlide: onLoadFailed bug here? ");
                            ...
                            public boolean onResourceReady(Bitmap resource, Object model, Target<Bitmap> target, DataSource dataSource,
                                                           boolean isFirstResource) {
                                Log.w(">>>>", "loadBitmapGlide: onResourceReady bug here? ");
     ...
     */
    @SuppressLint("CheckResult")
    private void loadFileGlide(boolean isDing, final int position, File file) {
        Log.w(">>>>", "loadFileGlide: bug here? ");
        RequestBuilder<Bitmap> requestBuilder =
                Glide.with(context.get())
                        .asBitmap()
                        .load(file)
                        ...
                        .listener(new RequestListener<Bitmap>() {
                            ...
                            public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Bitmap> target,
                                                        boolean isFirstResource) {
                                Log.w(">>>>", "loadFileGlide:onLoadFailed bug here? ");
                                Message message = mHandler.obtainMessage(LOAD_END, position, 0,
                                        BitmapFactory.decodeResource(context.get().getResources(), placeholder));
                                mHandler.sendMessage(message);
                                return false;
                            ...
                            public boolean onResourceReady(Bitmap resource, Object model, Target<Bitmap> target, DataSource dataSource,
                                                           boolean isFirstResource) {
                                Log.w(">>>>", "loadFileGlide:onResourceReady bug here? ");
                                Message message = mHandler.obtainMessage(LOAD_END, position, 0, resource);
    ...
    /**
     * 需要跟app中保持一直 是截取前两个还是后两个
     *
     * @return 获取昵称的简称
     */
    private static String getShortNickName(String userName) {
    ...
    private synchronized void showImage(Bitmap bitmap, final boolean fromCache) {
        mHandler.post(() -> {
            if (bitmap != null && context.get() != null && imageView.get() != null) {
                ...
                Log.w(">>>>", "showImage: requestBuilder");
                requestBuilder.into(imageView.get());
            }
        });
    }

    public Builder setReloadAvatar(boolean reloadAvatar) {
        this.reloadAvatar = reloadAvatar;
        return this;
    }

    public Builder setDiskFileListener(BitmapListener bitmapListener) {
