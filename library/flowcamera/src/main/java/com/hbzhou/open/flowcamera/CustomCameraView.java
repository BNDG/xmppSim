            ...
            public void onPictureTaken(@NonNull PictureResult result) {
                ...
                result.toFile(initTakePicPath(mContext), file -> {
                    if (file == null || !file.exists()) {
                        Toast.makeText(mContext, getContext().getString(R.string.file_not_exist), Toast.LENGTH_LONG).show();
                        return;
                    }
                    photoFile = file;
                    Glide.with(mContext)
...
//                mFlashLamp.setVisibility(VISIBLE);
        ...
        mCaptureLayout.setTypeLisenter(new TypeListener() {
            ...
            @Override
            public void confirm() {
                Log.d(">>>>", "mCameraView.getMode()>>> " + mCameraView.getMode());
...
//    }
    ...
    private void stopVideoPlay() {
        Log.d(">>>>", "stopVideoPlay here");
        if (mMediaPlayer != null) {
            Log.d(">>>>", "stopVideoPlay inner");
            mMediaPlayer.stop();
