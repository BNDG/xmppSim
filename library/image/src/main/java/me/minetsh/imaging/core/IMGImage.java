...
public class IMGImage {
    ...
    public IMGHoming clip(float scrollX, float scrollY) {
        ...
        // 设置裁剪后的最大尺寸
        MAX_SIZE = (int) (Math.max(mClipFrame.width(), mClipFrame.height()) * 3);
    ...
    public void toBackupClip() {
        ...
        M.mapRect(frame, mClipFrame);
        MAX_SIZE = (int) (Math.max(mClipFrame.width(), mClipFrame.height()) * 3);
        if (frame.contains(win) && !isRequestToBaseFitting) {
        } else {
            // 宽高都小于Win，才有必要放大
            if (isRequestToBaseFitting || frame.width() < win.width() && frame.height() < win.height()) {
                float scale = Math.min(win.width() / frame.width(), win.height() / frame.height());
                MAX_SIZE *= scale;
            }
        }
    }
...
//                    mClipWin
    ...
    public int onDrawMosaicsPath(Canvas canvas, Paint paint) {
        // 保存当前画布的状态并且创建一个新的图层，指定绘制的区域 mFrame，可以是一个矩形区域，也可以是整个画布区域。
        // saveLayer() 方法返回一个整型值 layerCount，该值是图层的 ID，绘制结束后可以使用 canvas.restore() 恢复到这个图层。
        ...
        if (!isMosaicEmpty()) {
            ...
            for (IMGPath path : mMosaics) {
                path.onDrawMosaic(canvas, paint);
            }
            canvas.restore();
        }
    ...
    public void onScale(float factor, float focusX, float focusY) {
        ...
        if (factor == 1f) return;
        float max = Math.max(mClipFrame.width(), mClipFrame.height());
        float min = Math.min(mClipFrame.width(), mClipFrame.height());
        if (max >= MAX_SIZE
                || min <= MIN_SIZE) {
            factor += (1 - factor) / 2;
        }
        if (max * factor > MAX_SIZE) {
            // 超出最大尺寸返回
            return;
        }
