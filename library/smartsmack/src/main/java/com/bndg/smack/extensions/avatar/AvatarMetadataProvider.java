...
*/
package com.bndg.smack.extensions.avatar;
                ...
                case START_ELEMENT:
                    ...
                    if (AvatarMetadata.Info.ELEMENT_INFO.equals(name)) {
                        ...
                        if ((id != null) && (type != null) && (bytes != 0))
                            info = new AvatarMetadata.Info(id, type, bytes);
                        else // invalid info
                            continue;

                        String url = parser.getAttributeValue(null, AvatarMetadata.Info.ATTR_URL);
                        info.setUrl(url);
                        ...
                        try {
                            ...
                            int height = 0;
                            if (sWidth != null)
                                width = Integer.parseInt(parser.getAttributeValue(null, AvatarMetadata.Info.ATTR_WIDTH));
                            if (sHeight != null)
                                height = Integer.parseInt(parser.getAttributeValue(null, AvatarMetadata.Info.ATTR_HEIGHT));
